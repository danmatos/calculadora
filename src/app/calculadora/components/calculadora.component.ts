...
export class CalculadoraComponent implements OnInit {
  ...
  /**
   * Adiciona número selecionado para calculo posterior.
   *
   * @param string numero
   * @return void
   */
  ...
  concatenarNumero(numAtual: string, numConcat: string): string {
    // if number is only 0 or null, restart the value
    if (numAtual === '0' || numAtual === null) {
      numAtual = '';
    }

    // if first digit is '.', concat '0' before the decimal point
    if (numConcat === '.' && numAtual === '') {
      return '0.';
    }

    // case '.' is typed and already contains '.' just return
  ...
  definirOperacao(operacao: string): void {
    // set the operations in case operations didn't exist
    if (this.operacao === null) {
      this.operacao = operacao;
      return;
    }

    /* in case of the operation is defined and the second
       number the same, conclude the operation */
    if (this.numero2 !== null) {
